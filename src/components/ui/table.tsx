...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b border-border/70 transition-[background-color,box-shadow] hover:bg-muted/55 data-[state=selected]:bg-muted dark:hover:bg-slate-900/50",
        className
      )}
      {...props}
    />
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-10 px-3 text-left align-middle text-[12px] font-semibold leading-4 whitespace-nowrap text-muted-foreground [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]",
        className
      )}
      {...props}
    />
