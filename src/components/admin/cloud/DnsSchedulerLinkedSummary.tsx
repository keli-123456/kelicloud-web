...
export default function DnsSchedulerLinkedSummary({
  ...
  return (
    <div className={cn("rounded-md border border-slate-200/70 bg-background/70 px-3 py-2.5 dark:border-slate-800 dark:bg-slate-950/20", className)}>
      ...
      {loading ? (
        ...
        <div className="mt-2 space-y-2">
          <div className="flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
            ...
            <span className="min-w-0 truncate font-mono text-[11px] text-foreground" title={renderIPPair(item)}>
              {renderIPPair(item)}
            </span>
          </div>
          <details className="group rounded-md border border-dashed border-border/80 px-2.5 py-1.5 text-xs dark:border-slate-800">
            <summary className="cursor-pointer select-none text-[11px] font-medium text-muted-foreground transition hover:text-foreground">
              {t("cloud.dns.scheduler.linked_detail_toggle", { defaultValue: "查看 DNS 目标和服务商" })}
            </summary>
            <div className="mt-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
              {[
                {
                  label: t("cloud.dns.scheduler.detail_dns_target", { defaultValue: "DNS 目标" }),
                  value: item.record_key || "-",
                },
                {
                  label: t("cloud.dns.scheduler.detail_provider", { defaultValue: "服务商" }),
                  value: `${formatProvider(item.provider, t)} / ${item.entry_id || "-"}`,
                },
                {
                  label: t("cloud.dns.scheduler.table.synced_at", { defaultValue: "最近时间" }),
                  value: formatDateTime(item.last_synced_at || item.updated_at),
                },
                {
                  label: t("cloud.dns.scheduler.detail_merge_group", { defaultValue: "合并组" }),
                  value: mergeGroupLabel(item, t),
                },
              ].map((row) => (
                <div key={row.label} className="min-w-0">
                  <div className="text-[11px] text-muted-foreground">{row.label}</div>
                  <div className="truncate font-medium text-foreground" title={row.value}>{row.value}</div>
                </div>
              ))}
            </div>
          </details>
