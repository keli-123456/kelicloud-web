        ...
        {allowClear && value ? (
          <div className="border-b px-2 py-1.5">
            ...
            >
              ...
              <span className="truncate">{clearLabel || "Clear selection"}</span>
            </button>
          </div>
        ) : null}
        <div
...
function azureImageReferenceFromPreset(preset: AzureImagePreset = initialAzureImagePreset): AzureImageReference {
  return {
...
function normalizeAzureImageReference(payload: Record<string, unknown>): AzureImageReference {
  const imageRecord = asRecord(payload.image);
  return {
    publisher: getStringValue(imageRecord?.publisher) || getStringValue(payload.image_publisher),
    offer: getStringValue(imageRecord?.offer) || getStringValue(payload.image_offer),
    sku: getStringValue(imageRecord?.sku) || getStringValue(payload.image_sku),
    version: getStringValue(imageRecord?.version) || getStringValue(payload.image_version) || "latest",
  };
}

function findAzureImagePreset(payload: Record<string, unknown>) {
  const presetID = getStringValue(payload.image_preset);
  if (presetID) {
    const preset = azureImagePresets.find((item) => item.id === presetID);
    if (preset) {
      return preset;
    }
  }
  ...
  return azureImagePresets.find((item) =>
    item.publisher === image.publisher
    && item.offer === image.offer
    && item.sku === image.sku
    && (item.version || "latest") === (image.version || "latest"))
    || initialAzureImagePreset;
}

function buildAzureImagePresetOptions() {
  return azureImagePresets.map((preset) => ({
    value: preset.id,
    label: preset.label,
    hint: `${preset.publisher} / ${preset.offer} / ${preset.sku}`,
  })) satisfies FailoverCatalogOption[];
}
...
function defaultPlanPayload(provider: string, actionType: string) {
  ...
  if (provider === "azure") {
    const image = azureImageReferenceFromPreset();
    return {
      name: "",
      resource_group: "",
...
function mergeAzurePlanCatalogWithCommon(
  catalog: FailoverPlanCatalog | null,
  region = "",
) {
  const commonCatalog = buildCommonAzurePlanCatalog(region);
  if (!catalog || catalog.provider !== "azure") {
    return commonCatalog;
  }

  return {
    ...catalog,
    region: catalog.region || region || DEFAULT_AZURE_REGION,
    regions: mergeCatalogOptions(commonCatalog.regions, catalog.regions),
    sizes: mergeCatalogOptions(commonCatalog.sizes, catalog.sizes),
    images: mergeCatalogOptions(commonCatalog.images, catalog.images),
  } satisfies FailoverPlanCatalog;
...
function validatePlanPayload(
  ...
  if (provider === "azure" && actionType === "provision_instance") {
    const image = normalizeAzureImageReference(payload);
    requirePlanField(t, index, t("failover.editor.region", { defaultValue: "Region" }), payload.region);
...
function normalizePlanPayloadForSubmit(
  ...
  if (provider === "azure" && actionType === "provision_instance") {
    const rootPassword = getStringValue(nextPayload.root_password) || getStringValue(nextPayload.admin_password);
    const image = normalizeAzureImageReference(nextPayload);
    const preset = findAzureImagePreset(nextPayload);

    nextPayload.name = getStringValue(nextPayload.name);
    nextPayload.resource_group = getStringValue(nextPayload.resource_group);
    nextPayload.region = getStringValue(nextPayload.region) || DEFAULT_AZURE_REGION;
    nextPayload.location = getStringValue(nextPayload.location) || getStringValue(nextPayload.region);
    nextPayload.size = getStringValue(nextPayload.size) || DEFAULT_AZURE_SIZE;
    nextPayload.admin_username = getStringValue(nextPayload.admin_username) || DEFAULT_AZURE_ADMIN_USERNAME;
    nextPayload.admin_password = rootPassword;
    nextPayload.root_password = rootPassword;
    nextPayload.root_password_mode = rootPassword ? "custom" : "random";
    nextPayload.ssh_public_key = getStringValue(nextPayload.ssh_public_key);
    nextPayload.user_data = typeof nextPayload.user_data === "string" ? String(nextPayload.user_data).trim() : "";
    nextPayload.public_ip = getBooleanValue(nextPayload.public_ip, true);
    nextPayload.assign_ipv6 = getBooleanValue(nextPayload.assign_ipv6, true);
    nextPayload.image_preset = preset.id;
    nextPayload.image = {
      publisher: image.publisher || preset.publisher,
      offer: image.offer || preset.offer,
      sku: image.sku || preset.sku,
      version: image.version || preset.version || "latest",
    };
    return nextPayload;
  }
...
function normalizePlanProviderEntryID(provider: string, entryID: string) {
  ...
  const normalizedEntryID = normalizeProviderEntryID(String(entryID || "").trim());
  if (
    ["aws", "azure", "digitalocean", "linode"].includes(normalizedProvider)
...
function TaskEditorDialog({
  ...
  const isAzureProvisionPlan = selectedPlan?.provider === "azure" && selectedPlan.action_type === "provision_instance";
  const azurePlanCatalog = React.useMemo(
    () => (
      isAzureProvisionPlan
        ? mergeAzurePlanCatalogWithCommon(planCatalog, selectedPlanRegion)
        : null
    ),
    [isAzureProvisionPlan, planCatalog, selectedPlanRegion],
  );
  ...
  const azureRegionOptions = React.useMemo(
    () => appendCatalogOptionIfMissing(azurePlanCatalog?.regions || [], selectedPlanRegion || DEFAULT_AZURE_REGION),
    [azurePlanCatalog, selectedPlanRegion],
  );
  const azureSizeOptions = React.useMemo(
    () => appendCatalogOptionIfMissing(azurePlanCatalog?.sizes || [], selectedPlanSize || DEFAULT_AZURE_SIZE),
    [azurePlanCatalog, selectedPlanSize],
  );
  const azureImageOptions = React.useMemo(
    () => appendCatalogOptionIfMissing(
      azurePlanCatalog?.images || buildAzureImagePresetOptions(),
      findAzureImagePreset(selectedPlanPayload).id,
    ),
    [azurePlanCatalog, selectedPlanPayload],
  );
  const selectedAzureImagePresetID = React.useMemo(
    () => findAzureImagePreset(selectedPlanPayload).id,
    [selectedPlanPayload],
  );
                  ...
                  <div className="space-y-4">
                    <div className="grid items-start gap-3 md:grid-cols-[minmax(0,1fr)_18rem]">
                      <div className={FORM_FIELD_CLASS}>
                        <Label htmlFor="failover-name" className="flex items-center gap-1">
                      ...
                      <div className={FORM_TOGGLE_CLASS}>
                        <div className="min-w-0">
                          ...
                          <div className="text-xs leading-5 text-slate-500 dark:text-slate-400">
                            {t("failover.editor.enabled_hint", { defaultValue: "Disabled tasks keep their configuration but will not run automatically." })}
                          </div>
                        </div>
                        <Switch
                    ...
                    <div className={FORM_FIELD_CLASS}>
                      ...
                      <SearchableCatalogSelect
                        ...
                        placeholder={t("failover.editor.current_client_placeholder", { defaultValue: "Choose a current client, optional" })}
                        ...
                        allowClear
                        clearLabel={t("failover.editor.current_client_clear", { defaultValue: "Clear current client" })}
                      />
                      <div className="text-xs text-muted-foreground">
                ...
                <DialogContent className={cn(ADMIN_FORM_DIALOG_WIDE_CLASS, ADMIN_FORM_DIALOG_CHROME_CLASS, "h-[88vh] max-w-[1020px] sm:max-w-[1020px]")}>
                  ...
                  <div className={ADMIN_FORM_BODY_CLASS}>
                    ...
                    >
                          ...
                          {showPlanCatalogLoadActions ? (
                            ...
                            >
                              ...
                              {isDigitalOceanProvisionPlan
                                ...
                                : isAzureProvisionPlan
                                  ? t("failover.editor.load_plan_options_full_azure", { defaultValue: "Load Azure regions and sizes" })
                                  : isLinodeProvisionPlan
                            ...
                            {selectedPlan.action_type === "provision_instance" ? (
                              <div className="grid gap-3 border-y border-slate-200/80 py-3 dark:border-slate-800/80 sm:grid-cols-[minmax(0,1fr)_12rem] sm:items-center">
                                <div className="min-w-0 space-y-1">
                                  <Label>{t("failover_v2.candidate_count", { defaultValue: "Candidate instances" })}</Label>
                                  <p className="text-xs leading-5 text-muted-foreground">
                      ...
                      {selectedPlan.provider === "azure" && selectedPlan.action_type === "provision_instance" ? (
                        <div className="space-y-5">
                          <div className="space-y-3">
                            <div className="min-w-0">
                              <div className="text-sm font-medium text-slate-900 dark:text-slate-50">
                                {t("failover.editor.azure_compute_section", { defaultValue: "Azure VM" })}
                              </div>
                              <div className="mt-1 text-xs leading-5 text-muted-foreground">
                                {t("failover.editor.azure_compute_section_hint", {
                                  defaultValue: "Choose the location, VM size, and Linux image used when failover creates a replacement outlet.",
                                })}
                              </div>
                            </div>
                            <div className="grid gap-4 lg:grid-cols-2">
                              <div className="space-y-2">
                                <Label>{t("failover.editor.region", { defaultValue: "Region" })}</Label>
                                <SearchableCatalogSelect
                                  value={getStringValue(selectedPlanPayload.region) || DEFAULT_AZURE_REGION}
                                  ...
                                  searchPlaceholder={t("failover.editor.region_search_placeholder", { defaultValue: "Search regions..." })}
                                  emptyLabel={t("failover.editor.region_search_empty", { defaultValue: "No matching region" })}
                                  formatOptionLabel={(option) => formatPlanRegionOptionLabel(t, selectedPlan.provider, option)}
                                />
                              </div>
                              <div className="space-y-2">
                                <Label>{t("failover.editor.size", { defaultValue: "Size" })}</Label>
                                <SearchableCatalogSelect
                                  value={getStringValue(selectedPlanPayload.size) || DEFAULT_AZURE_SIZE}
                              ...
                              <div className="space-y-2 lg:col-span-2">
                                <Label>{t("failover.editor.image", { defaultValue: "Image" })}</Label>
                                <SearchableCatalogSelect
                                  value={selectedAzureImagePresetID}
                                  ...
                                  onValueChange={(value) => {
                                    ...
                                    updateSelectedPlanPayload((current) => ({
                                      ...
                                      image: azureImageReferenceFromPreset(preset),
                                    }));
                                  }}
                                  placeholder={t("failover.editor.image_placeholder", { defaultValue: "Choose an image" })}
                                  searchPlaceholder={t("failover.editor.image_search_placeholder", { defaultValue: "Search images..." })}
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="grid gap-4 lg:grid-cols-2">
                              ...
                              <div className="space-y-2 lg:col-span-2">
                                ...
                                <Textarea
                                  className="min-h-20 font-mono text-xs [overflow-wrap:anywhere]"
                                  value={getStringValue(selectedPlanPayload.ssh_public_key)}
                                  onChange={(event) => updateSelectedPlanPayload((current) => ({
                                    ...current,
                        ...
                        >
                          ...
                          <td className="min-w-0 overflow-hidden px-3 py-2 align-middle">
                            <div className="flex min-w-0 items-center gap-1.5 whitespace-nowrap">
                              ...
                              {platformAdmin ? (
                                <span className="hidden min-w-0 truncate text-[12px] leading-5 text-muted-foreground 2xl:inline" title={view.dnsTargetLabel || undefined}>
